...
def excel_date(date):
    """
    Convert a Python datetime.date object to Excel serial date format.
    Excel serial dates are days since 1900-01-01, with 1900 treated as a leap year.
    """
    excel_epoch = datetime(1899, 12, 30)  # Excel's date system starts here
    delta = date - excel_epoch.date()
    return delta.days
...
def update_excel(df, codes, date, transmittal, date_col, transmittal_col, code_col):
    ...
    # Convert the input date to Excel serial date format
    excel_date_value = excel_date(date)
    ...
    for code in codes:
        ...
        if not matching_rows.empty:
            df.loc[mask, date_col] = excel_date_value
            ...
            if st.button("Update Data"):
                if not codes_input:
                    ...
                    if updated_rows > 0:
                        ...
                        mask = (updated_df[date_col] == excel_date(date_value)) & (updated_df[transmittal_col] == transmittal_value)
